...
为 `xianyu_goods_info` 表添加 `xianyu_account_id` 字段，用于关联闲鱼账号，实现商品与账号的绑定关系。
...
-- 索引
CREATE UNIQUE INDEX idx_goods_xy_good_id ON xianyu_goods_info(xy_good_id);
CREATE INDEX idx_goods_status ON xianyu_goods_info(status);
CREATE INDEX idx_goods_account_id ON xianyu_goods_info(xianyu_account_id);
```
